...
public class BootstrapUsers {
	...
	public void initializeDBWithAppUsers() {
		...
		studentAppUserRepository.save(new StudentAppUser("abcham", passwordEncoder.encode("password"), "admin@email.net", "Adminis Trator", ADMIN, ADMIN.grantedAuthorities(), true, true, true, true));
		studentAppUserRepository.save(new StudentAppUser("hcham", passwordEncoder.encode("password"), "admin@email.net", "Admin Assistant", ADMIN_TRAINEE, ADMIN_TRAINEE.grantedAuthorities(), true, true, true, true));
		studentAppUserRepository.save(new StudentAppUser("asimaha", passwordEncoder.encode("password"), "student@email.net", "Student Student", STUDENT, STUDENT.grantedAuthorities(), true, true, true, true));
